...
app=FastAPI(title='UNG-CONSTELLATION',version='1.5.0')
...
@app.get('/')
def root(): return {'service':'UNG-CONSTELLATION','name':'National Satellite & Orbital Operations System','version':'1.5.0','no_transmit':NO_TRANSMIT,'mission_control':'/mission-control'}
@app.get('/health')
...
@app.get('/v1/system')
def system(): return {'service':'UNG-CONSTELLATION','mission':'Satellite tracking, orbital operations, ground-station coordination and telemetry','version':'1.5.0','integrations':integrations.status(),'no_transmit':NO_TRANSMIT}
...
def mission_control():
 return '''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>UNG-CONSTELLATION</title><style>*{box-sizing:border-box}body{font-family:system-ui;background:#06111d;color:#e8f1fb;margin:0}.bar{padding:18px;background:#0c1b2a}.wrap{padding:14px;max-width:1200px;margin:auto}.card{background:#0c1b29;border:1px solid #24506d;border-radius:16px;padding:16px;margin-bottom:12px}.ok{color:#68e3a1}.warn{color:#ffd166}.muted{color:#91a8bb}.search{display:flex;gap:8px;flex-wrap:wrap}input{flex:1;min-width:140px;background:#07131f;color:#fff;border:1px solid #315b77;border-radius:9px;padding:11px}button{padding:11px 14px;border:0;border-radius:9px;font-weight:700}#map{width:100%;height:360px;background:#07131f;border-radius:12px;position:relative;overflow:hidden}canvas{width:100%;height:100%}.metric{font-size:20px;font-weight:700}</style></head><body><div class="bar"><h2>UNG-CONSTELLATION · Mission Control</h2><div class="muted">Orbital Tracking · Ground Track · Receive-Only Doppler</div></div><div class="wrap"><div class="card"><b>RF SAFETY: <span class="warn">NO TRANSMIT</span></b> · UGANET-GS-001</div><div class="card"><h3>Satellite Operations</h3><div class="search"><input id="q" placeholder="Satellite name"><button onclick="searchSat()">SEARCH</button></div><div id="results" class="muted"></div><div class="search" style="margin-top:10px"><input id="norad" placeholder="NORAD ID"><input id="lat" type="number" step="any" placeholder="Station latitude"><input id="lon" type="number" step="any" placeholder="Station longitude"><input id="freq" type="number" step="1" placeholder="Receive frequency Hz"><button onclick="track()">TRACK</button></div><p id="status" class="muted">Select a satellite and enter station coordinates.</p></div><div class="card"><h3>Live Receive Doppler</h3><div id="doppler" class="metric muted">Enter a receive frequency to calculate Doppler correction.</div></div><div class="card"><h3>Orbital Map</h3><div id="map"><canvas id="cv"></canvas></div></div><div class="card"><h3>Upcoming Passes</h3><div id="passes" class="muted">Waiting for tracking selection.</div></div></div><script>
let timer=null;
