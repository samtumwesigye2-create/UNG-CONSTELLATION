...
from .security import require_role, require_edge_key
from .tracker_api import router as tracker_router
app=FastAPI(title='UNG-CONSTELLATION',version='1.10.0');NO_TRANSMIT=os.getenv('CONSTELLATION_NO_TRANSMIT','true').lower() not in ('0','false','no')
...
@app.get('/')
def root():return {'service':'UNG-CONSTELLATION','version':'1.10.0','no_transmit':NO_TRANSMIT,'mission_control':'/mission-control','sat_tracker_engine':'/v1/tracker/snapshot'}
@app.get('/health')
...
@app.get('/v1/system')
def system():return {'service':'UNG-CONSTELLATION','version':'1.10.0','integrations':integrations.status(),'no_transmit':NO_TRANSMIT,'sat_tracker_paired':True,'edge_auth':True,'exact_replay':True}
...
@app.get('/v1/receptions/{archive_id}/samples')
def reception_samples(archive_id:int,limit:int=1000):
 out=db.archive_samples(archive_id,limit)
 if out is None:raise HTTPException(404,'Reception archive not found')
 return out
