...
app=FastAPI(title='UNG-CONSTELLATION',version='1.7.0');NO_TRANSMIT=os.getenv('CONSTELLATION_NO_TRANSMIT','true').lower() not in ('0','false','no')
@app.on_event('startup')
def startup():
    try:db.init_db()
    except Exception:pass
@app.get('/')
def root():return {'service':'UNG-CONSTELLATION','version':'1.7.0','no_transmit':NO_TRANSMIT,'mission_control':'/mission-control'}
@app.get('/health')
...
@app.get('/v1/system')
def system():return {'service':'UNG-CONSTELLATION','version':'1.7.0','integrations':integrations.status(),'no_transmit':NO_TRANSMIT}
...
def mc():return '''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>UNG-CONSTELLATION</title><style>body{font-family:system-ui;background:#06111d;color:#e8f1fb;margin:0}.w{max-width:1200px;margin:auto;padding:14px}.c{background:#0c1b29;border:1px solid #24506d;border-radius:15px;padding:15px;margin:12px 0}.m{color:#91a8bb}.warn{color:#ffd166}input,button{padding:10px;margin:3px;border-radius:8px;border:1px solid #315b77;background:#07131f;color:white}button{font-weight:700}.g{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.x{background:#07131f;padding:10px;border-radius:9px}canvas{width:100%;height:300px;background:#07131f;border-radius:10px}@media(max-width:700px){.g{grid-template-columns:1fr 1fr}}</style></head><body><div class="w"><h2>UNG-CONSTELLATION · Mission Control</h2><div class="c"><b>RF SAFETY <span class="warn">NO TRANSMIT</span></b> · UGANET-GS-001</div><div class="c"><h3>Satellite Tracking</h3><input id="q" placeholder="Satellite"><button id="search">SEARCH</button><div id="res"></div><input id="norad" placeholder="NORAD"><input id="lat" placeholder="Latitude"><input id="lon" placeholder="Longitude"><input id="freq" placeholder="Receive Hz"><button id="track">TRACK</button><p id="status" class="m"></p></div><div class="c"><h3>RF Signal</h3><div class="g"><div class="x">EDGE<br><b id="edge">WAITING</b></div><div class="x">SNR<br><b id="snr">--</b></div><div class="x">PEAK<br><b id="peak">--</b></div><div class="x">NOISE<br><b id="noise">--</b></div></div></div><div class="c"><h3>Live Spectrum</h3><canvas id="sp"></canvas></div><div class="c"><h3>30-Minute RF Waterfall</h3><canvas id="wf"></canvas><p class="m">Newest samples at bottom · PostgreSQL history</p></div><div class="c"><h3>Orbital Map</h3><canvas id="map"></canvas></div><div class="c"><h3>Upcoming Passes</h3><div id="passes" class="m"></div></div></div><script>const E=id=>document.getElementById(id);let timer;function spectrum(c,b){let x=c.getContext('2d'),r=c.getBoundingClientRect(),w=r.width,h=r.height;c.width=w*devicePixelRatio;c.height=h*devicePixelRatio;x.scale(devicePixelRatio,devicePixelRatio);x.fillStyle='#07131f';x.fillRect(0,0,w,h);if(!b||b.length<2)return;let v=b.map(z=>+z.power_db),mn=Math.min(...v),mx=Math.max(...v);x.strokeStyle='#68e3a1';x.beginPath();b.forEach((z,i)=>{let xx=i/(b.length-1)*w,yy=h-(+z.power_db-mn)/(mx-mn||1)*(h-20)-10;i?x.lineTo(xx,yy):x.moveTo(xx,yy)});x.stroke()}function waterfall(s){let c=E('wf'),x=c.getContext('2d'),r=c.getBoundingClientRect(),w=r.width,h=r.height;c.width=w*devicePixelRatio;c.height=h*devicePixelRatio;x.scale(devicePixelRatio,devicePixelRatio);x.fillStyle='#07131f';x.fillRect(0,0,w,h);let all=s.flatMap(a=>(a.bins||[]).map(b=>+b.power_db));if(!all.length)return;let mn=Math.min(...all),mx=Math.max(...all),rh=h/s.length;s.forEach((a,j)=>(a.bins||[]).forEach((z,i)=>{let n=(+z.power_db-mn)/(mx-mn||1);x.fillStyle='hsl('+(240-240*n)+',90%,'+(25+40*n)+'%)';x.fillRect(i/a.bins.length*w,j*rh,w/a.bins.length+1,rh+1)}))}async function rf(){try{let j=await fetch('/v1/edge/nodes').then(r=>r.json()),n=(j.nodes||[])[0],s=(((n||{}).health||{}).rf||{}).spectrum||{};E('edge').textContent=s.ok?'ONLINE':'WAITING';E('snr').textContent=s.ok?(+s.snr_db).toFixed(1)+' dB':'--';E('peak').textContent=s.ok?(+(s.peak||{}).power_db).toFixed(1)+' dB':'--';E('noise').textContent=s.ok?(+s.noise_floor_db).toFixed(1)+' dB':'--';spectrum(E('sp'),s.bins||[]);let h=await fetch('/v1/rf/history?minutes=30&limit=180').then(r=>r.json());waterfall(h.samples||[])}catch(e){E('edge').textContent='OFFLINE'}}E('search').onclick=async()=>{let j=await fetch('/v1/catalog/search?q='+encodeURIComponent(E('q').value)).then(r=>r.json());E('res').innerHTML=(j.satellites||[]).map(s=>'<div data-n="'+s.norad_id+'">'+s.name+' · '+s.norad_id+'</div>').join('');E('res').querySelectorAll('[data-n]').forEach(d=>d.onclick=()=>E('norad').value=d.dataset.n)};function omap(points,p,la,lo){let c=E('map'),x=c.getContext('2d'),r=c.getBoundingClientRect(),w=r.width,h=r.height;c.width=w*devicePixelRatio;c.height=h*devicePixelRatio;x.scale(devicePixelRatio,devicePixelRatio);x.fillStyle='#07131f';x.fillRect(0,0,w,h);x.strokeStyle='#68e3a1';x.beginPath();let last=null;points.forEach(z=>{let xx=(z.lon+180)/360*w,yy=(90-z.lat)/180*h;if(last!==null&&Math.abs(xx-last)>w/2){x.stroke();x.beginPath()}x.lineTo(xx,yy);last=xx});x.stroke()}E('track').onclick=async function go(){let id=E('norad').value,la=+E('lat').value,lo=+E('lon').value;if(!id)return;let t=await fetch('/v1/catalog/track?norad_id='+id+'&lat='+la+'&lon='+lo).then(r=>r.json()),g=await fetch('/v1/catalog/ground-track?norad_id='+id).then(r=>r.json()),p=t.position;E('status').textContent=t.satellite+' · AZ '+p.azimuth_deg.toFixed(1)+'° · EL '+p.elevation_deg.toFixed(1)+'° · RANGE '+p.range_km.toFixed(0)+' km';omap(g.points||[],p,la,lo);E('passes').innerHTML=(t.passes||[]).slice(0,8).map(v=>v.rise.time+' · MAX '+Number(v.max_elevation_deg).toFixed(1)+'°<br>').join('');let f=+E('freq').value;if(f>0){let d=await fetch('/v1/catalog/doppler?norad_id='+id+'&lat='+la+'&lon='+lo+'&frequency_hz='+f).then(r=>r.json());E('status').textContent+=' · RX '+(d.doppler.receive_frequency_hz/1e6).toFixed(6)+' MHz'}clearTimeout(timer);timer=setTimeout(go,5000)};rf();setInterval(rf,10000)</script></body></html>'''
@app.post('/v1/tle')
