...
app=FastAPI(title='UNG-CONSTELLATION',version='1.8.0');NO_TRANSMIT=os.getenv('CONSTELLATION_NO_TRANSMIT','true').lower() not in ('0','false','no')
@app.on_event('startup')
def startup():
 try:db.init_db()
 except Exception:pass
@app.get('/')
def root():return {'service':'UNG-CONSTELLATION','version':'1.8.0','no_transmit':NO_TRANSMIT,'mission_control':'/mission-control'}
@app.get('/health')
...
@app.get('/v1/system')
def system():return {'service':'UNG-CONSTELLATION','version':'1.8.0','integrations':integrations.status(),'no_transmit':NO_TRANSMIT}
